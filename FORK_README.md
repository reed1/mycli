...
- **How it works** - When visidata-db is launched as the pager, mycli starts a per-process Unix-domain **socket server** on a daemon thread and exports its path via the `DB_SOCKET` environment variable. visidata-db connects to that socket and sends a newline-terminated JSON request (`{"action": "drill_up"|"drill_down"|"open_table", ...}`); mycli runs the corresponding drill query on the live connection and returns CSV. Because mycli's main thread is blocked inside the pager while it is open, the socket handler can safely reuse the live database connection.
...
- **`DB_SOCKET`** - Set by mycli (not the user) to the path of the per-process drill socket while visidata-db is open; visidata-db reads it to send live drill/open-table requests back to the CLI
